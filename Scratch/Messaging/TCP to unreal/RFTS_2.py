...
class DataInlet(Inlet):  
    ...
    def find_events(self):
        global filtraw,fs,events_dict,events,unicorn,feat,fevents
        events = mne.find_events(filtraw,stim_channel = "Stim")
        print(events[:10])
        if feat:
            fevents = mne.find_events(feat,stim_channel = "Stim")
        # Trial Descriptor
        events_dict={
            "attention":1000,
            "relax":2000
        ...
        if (unicorn):
            ...
            feat.plot(fevents,event_color={1000:"r",2000:"g",100:"b",96:"y"},title="Band Events-non scale")
        else:
            filtraw.plot(events,event_color={1000:"r",2000:"g",100:"b",96:"y"},scalings="auto",title="Filtered Events")
            feat.plot(fevents,event_color={1000:"r",2000:"g",100:"b",96:"y"},scalings="auto",title="Band Events")

    def epoch_plot(self):
        global filtraw,events,events_dict,unicorn,att_epo,rel_epo,x_train,y_train,epochs,fs,loadraw,raw_info,feat,fevents,x2_train,fepochs
        epochs = mne.Epochs(filtraw, events, event_id=events_dict, preload=True)
        fepochs = mne.Epochs(feat, fevents, event_id=events_dict, preload=True)
        # epochs.plot(
        ...
        att_epo = epochs["attention"].get_data()
        rel_epo = epochs["relax"].get_data()
        ...
        # compute psd for each epoch and plot as subplots

        # for i in epochs["attention"]:
        #     att_psd=i.compute_psd()
        #     att_psd.plot(average=False,picks='data', exclude='bads')
        #     plt.title("Attention PSD")

        # for ii in epochs["relax"]:
        #     # rlx_psd=epochs["relax"].compute_psd()
        #     rlx_psd=i.compute_psd()
        #     rlx_psd.plot(average=False,picks='data', exclude='bads')
        #     plt.title("Relax PSD")


        # frequencies = np.arange(7, 30, 3)
        # power = mne.time_frequency.tfr_morlet(
        #     epochs["attention"], n_cycles=2, return_itc=False, freqs=frequencies, decim=3
        # )
        # power.plot()
        # power = mne.time_frequency.tfr_morlet(
        #     epochs["relax"], n_cycles=2, return_itc=False, freqs=frequencies, decim=3
        # )
        # power.plot()
    ...
    def hilbert_plot(self):
        ...
        global filtraw,fs,att_epo,rel_epo
        attx=att_epo[:,0,:]   # this indexing
        rlxx=rel_epo[:,0,:] 
        # print(attx)
        attx=attx.flatten()
        rlxx=rlxx.flatten()
        ...
        fig.tight_layout()
        plt.show()
        # IP, IF, I A = emd.spectra.frequency_transform(imf, fs, 'hilbert')
    ...
    def save_exp(self):
        ...
        # hil=filtraw.apply_hilbert()
        # hiln=filrawn.apply_hilbert()
        # hil.plot()
        # hiln.plot()
    # def fft_epoch(self):
    def loadraw(self):
    ...
    def clf_logreg(self):
        global x_train,y_train,epochs,x2_train,fepochs
        clf = make_pipeline(Scaler(epochs.info),
                            Vectorizer(),
                            LogisticRegression(solver="liblinear"))
        fclf = make_pipeline(Scaler(fepochs.info),
                            Vectorizer(),
                            LogisticRegression(solver="liblinear"))
        ...
        fscores = cross_val_multiscore(fclf,x2_train,y_train, cv=5, n_jobs=1)
        fscore = np.mean(scores,axis=0) # cross validation score

        print("Score: %0.2f%%" % (100 * score,))
        print("F-Score: %0.2f%%" % (100 * fscore,))
    ...
    def discrete_wavelet(self):
        ...
        global filtraw
        dwtraw = filtraw.copy()
        dwatdata = dwtraw.get_data()[0]
        coeffs = pywt.wavedec(dwatdata, 'db4', level=6)
    ...
    def filter_bank(self):
        global fs,filtraw,max_epoch,feat
        
        bandraw = filtraw.copy()
        bandraw2 = bandraw.copy()

        time = bandraw.times
        bandrawA = bandraw.crop(tmax=time.max()/2)
        # bandraw.plot(show=True,title="RawBand")
        bandrawB = bandraw2.crop(tmin=time.max()/2)
        bandrawC = bandrawA.copy()
        bandrawD = bandrawB.copy()

        deltaA = bandrawA.filter(.1,4).get_data()[0]
        thetaA = bandrawA.filter(4,8).get_data()[0] 

        alpA = bandrawA.filter(8,13)
        alphaA = alpA.get_data()[0]
        betA = bandrawC.filter(13,32)
        betaA = betA.get_data()[0]

        gammaA = bandrawA.filter(32,50).get_data()[0]
        bandA = [deltaA, thetaA, alphaA, betaA, gammaA]

        deltaB = bandrawB.filter(.1,4).get_data()[0]
        thetaB = bandrawB.filter(4,8).get_data()[0]

        alpB = bandrawB.filter(8,13)
        alphaB = alpB.get_data()[0]
        betB = bandrawD.filter(13,32)
        betaB = betB.get_data()[0]

        gammaB = bandrawB.filter(32,50).get_data()[0]

        timeA = bandrawA.times
        timeB = bandrawB.times
        ...
        for i, b in enumerate (bandA):
            ax[i].plot(timeA,b,label=bandname[i])
...
def main():
    ...
    btn_panel.layout().addWidget(Hilbert_Button)
    btn_panel.layout().addWidget(Classifylog_Button)
    btn_panel.layout().addWidget(ClassiftSVM_Button)
    btn_panel.layout().addWidget(BandPower_Button)
    btn_panel.layout().addWidget(DWavelets_Button)
    ...
    btn_panel.layout().addWidget(UseLoadedRaw_Button)
